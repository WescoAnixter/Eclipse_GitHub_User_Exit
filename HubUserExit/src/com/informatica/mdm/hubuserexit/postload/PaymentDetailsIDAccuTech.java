...
public class PaymentDetailsIDAccuTech 
{

	public void PaymentDetailMethodAT(String rowIdobject, Statement stmt, String prtyRowIdobject, String tablename) 
	{
		Logger log = Logger.getLogger(PaymentDetailsIDAccuTech.class);
		
		{
			log.info("rowIdobject PaymentDetailsIdAT - " + rowIdobject + "prtyRowIdobject - " + prtyRowIdobject + "tablename - " + tablename);
			ResultSet Locrs = null;
			ResultSet Pmntrs = null;
			
			try {
				
				Pmntrs = stmt.executeQuery("SELECT X_PMNT_DTLS_ID PMNT_DTLS_ID FROM SUPPLIER_HUB.C_XO_PMNT_DTLS_ACCU_TECH WHERE HUB_STATE_IND = '1' AND ROWID_OBJECT ="
								+ rowIdobject + "");
				...
				while (Pmntrs.next()) 
				{
					...
					log.info("PMNT_DTLS_ID_AT " + PmntDtlsId);
				}
				
				Locrs = stmt.executeQuery("SELECT X_PMNT_NM PMNT_NM FROM SUPPLIER_HUB.C_XO_PMNT_DTLS_ACCU_TECH WHERE HUB_STATE_IND = '1' AND X_PMNT_DTLS_ID ='"
								+ PmntDtlsId + "'");
				...
				while (Locrs.next()) 
				{
					...
					log.info("PMNT_NM_AT " + PmntNm);
				}
				
				//PmntNm = PmntNm.replace("&", "&amp;");
				//log.info("LocNm after replace - " + PmntNm);
				
				log.info("X_PMNT_DTLS_ID_AT " + PmntDtlsId);
				log.info("X_PMNT_NM_AT " + PmntNm);
				
				ResultSet newPmntrs = null;
				
				newPmntrs = stmt.executeQuery("SELECT X_PMNT_ID_ACCU_TECH PMNT_DTLS_ID FROM SUPPLIER_HUB.C_XT_PMNT_ID_ACCU_TECH WHERE X_PMNT_ID_ACCU_TECH ='"
								+ PmntDtlsId + "'");
				
				//log.info("SELECT X_PMNT_DTLS_ID PMNT_DTLS_ID FROM SUPPLIER_HUB.C_XT_PMNT_ID_ACCU_TECH WHERE HUB_STATE_IND = 1 AND X_PMNT_DTLS_ID ='"+ PmntDtlsId + "'");
				
				//log.info("newPmntrs Result set value ---> " + newPmntrs);
				
				String newPmntDtlsId = null;
				while (newPmntrs.next()) 
				{
					newPmntDtlsId = newPmntrs.getString("PMNT_DTLS_ID");
					log.info("PMNT_DTLS_ID of C_XT_PMNT_ID_ACCU_TECH  --> " + newPmntDtlsId);
				}
			
				if(newPmntDtlsId != null)
				{

					log.info("PMNT_LOC_NM of C_XO_PMNT_DTLS_ACCU_TECH  --> " + PmntNm);
					stmt.executeUpdate("UPDATE SUPPLIER_HUB.C_XT_PMNT_ID_ACCU_TECH SET X_PRTY_FK='"+ prtyRowIdobject + "', X_PMNT_LOC_NM_ACCU_TECH ='"+ PmntNm +"' WHERE X_PMNT_ID_ACCU_TECH = '" + PmntDtlsId + "'");
					stmt.executeUpdate("UPDATE SUPPLIER_HUB.C_XT_PMNT_ID_ACCU_TECH_XREF SET X_PRTY_FK='"+ prtyRowIdobject + "',S_X_PRTY_FK='"+ prtyRowIdobject + "', X_PMNT_LOC_NM_ACCU_TECH ='"+ PmntNm +"' WHERE X_PMNT_ID_ACCU_TECH = '" + PmntDtlsId + "'");
							log.info("X_PMNT_LOC_NM AT updated.");
					
				}
				else if(newPmntDtlsId == null && PmntNm != null)
				{
					PmntNm = PmntNm.replace("&", "&amp;");
					...
					log.info("UPDATE SUPPLIER_HUB.C_XT_PMNT_ID_ACCU_TECH"
							+" SET X_PRTY_FK='"+prtyRowIdobject+"' WHERE X_PMNT_ID_ACCU_TECH = '" + PmntDtlsId + "'");
							
					stmt.executeUpdate("UPDATE SUPPLIER_HUB.C_XT_PMNT_ID_ACCU_TECH"
							+" SET X_PRTY_FK='"+prtyRowIdobject+"' WHERE X_PMNT_ID_ACCU_TECH = '" + PmntDtlsId + "'");
							
					log.info("After REL update");
							
					log.info("UPDATE SUPPLIER_HUB.C_XT_PMNT_ID_ACCU_TECH_XREF"
							+" SET X_PRTY_FK='"+prtyRowIdobject+"', S_X_PRTY_FK='"+prtyRowIdobject+"'WHERE X_PMNT_ID_ACCU_TECH = '" + PmntDtlsId + "'");
							
					stmt.executeUpdate("UPDATE SUPPLIER_HUB.C_XT_PMNT_ID_ACCU_TECH_XREF"
							+" SET X_PRTY_FK='"+prtyRowIdobject+"', S_X_PRTY_FK='"+prtyRowIdobject+"'WHERE X_PMNT_ID_ACCU_TECH = '" + PmntDtlsId + "'");
							
					log.info("After REL XREF update");
					log.info("Updated in C_XT_PMNT_ID_ACCU_TECH ");
				}
	...
	public Map<String, String> soappmntdtlslov(String pmntNm, String pmntDtlsId, String tablename) {
		...
		Logger log = Logger.getLogger(GeneratePrchsFrmId.class);
  		...
  		+ " <soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:urn=\"urn:siperian.api\">"
			...
			+ "</urn:stringValue>" + " <urn:name>X_PMNT_ID_ACCU_TECH</urn:name>" + " </urn:field>"
			+ " <urn:field>"
			+ " <urn:stringValue>" + pmntNm
			+ "</urn:stringValue>" + " <urn:name>X_PMNT_LOC_NM_ACCU_TECH</urn:name>" + " </urn:field>"
			+ " <urn:siperianObjectUid>BASE_OBJECT.C_XT_PMNT_ID_ACCU_TECH</urn:siperianObjectUid>"
		    ...
		    if (!tablename.equals("C_XT_PMNT_ID_ACCU_TECH")) {
